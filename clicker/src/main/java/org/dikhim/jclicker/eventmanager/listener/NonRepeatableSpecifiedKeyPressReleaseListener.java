...
public class NonRepeatableSpecifiedKeyPressReleaseListener extends SimpleListener implements KeyListener {
    private String key;
    private Consumer<KeyPressEvent> onPress;
    private Consumer<KeyReleaseEvent> onRelease;
    private PressedKeyHolder pressedKeyHolder = new PressedKeyHolder();

    public NonRepeatableSpecifiedKeyPressReleaseListener(String id, String key, Consumer<KeyPressEvent> onPress, Consumer<KeyReleaseEvent> onRelease) {
        super(id);
        this.key = key;
    ...
    public void keyPressed(KeyPressEvent event) {
        if (pressedKeyHolder.getPressedKeys().contains(event.getKey())) return;
        pressedKeyHolder.press(event.getKey());
    ...
    public void keyReleased(KeyReleaseEvent event) {
        if (!pressedKeyHolder.getPressedKeys().contains(event.getKey())) return;
        pressedKeyHolder.release(event.getKey());
